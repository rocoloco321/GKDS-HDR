...
extern "C"
{
	...
	void* heap_alloc(void* heap, int allocsize);
	int* proc_getHeap(void* proc);
	int* proc_getCurrent();
	u32 scproc_getCurrentScene();
	int race_race_getCurrentRaceMode();
	int race_race_getCurrentDisplayMode();
	void* driver_getById(int driverId);
	int scene_GX_SetMasterBrightness(int brightness);
	void title_playActivationSound(void);
